...
export class EvolutionAPIManager {
  ...
  constructor() {
    this.config = {
      ...
      webhookUrl: process.env.EVOLUTION_WEBHOOK_URL || 'http://localhost:5000/api/webhook/evolution'
    };
  ...
  async setupInstance(): Promise<boolean> {
    try {
      ...
      const response = await axios.post(`${this.config.baseURL}/instance/create`, payload, {
  ...
  private async checkInstanceExists(): Promise<boolean> {
    try {
      ...
      return instances.some((inst: any) => 
        inst.instance?.instanceName === this.config.instanceName
      );
    } catch (error) {
      return false;
    }
  ...
  async checkConnection(): Promise<boolean> {
    ...
    try {
      ...
      if (this.isConnected) {
        console.log(`✅ WhatsApp conectado - ${status.profileName || 'Perfil'}`);
      }
...
1️⃣ *Demonstração Gratuita (30min)*
   ...
   → Funcionalidades específicas para ${cliente.setor?.toLowerCase() || 'seu setor'}
...
${cliente.email ? `📧 *Relatório técnico completo* também enviado para: ${cliente.email}` : ''}
...
*Obrigado por confiar na nossa tecnologia!* 🎉✨`;
  ...
  async sendMessage(cliente: ClienteData): Promise<MensagemResult> {
    ...
    try {
      ...
      console.log(`📤 Enviando via Evolution API para: ${cliente.telefone} (${numeroFormatado})`);
  ...
  async sendMessageHybrid(cliente: ClienteData): Promise<MensagemResult> {
    ...
    try {
      ...
      return {
        ...
        // Incluir URL para o frontend usar
        whatsappUrl
  ...
  // 10. NOTIFICAR EQUIPA
  async notifyTeam(cliente: ClienteData, resultado: MensagemResult): Promise<void> {
...
📡 *MÉTODO DE ENVIO:* ${resultado.metodo === 'evolution' ? '✅ Evolution API (Automático)' : '📱 wa.me (Fallback)'}
...
🚨 *LEAD QUALIFICADO - SEGUIMENTO PRIORITÁRIO!*`;
        ...
        case 'messages.upsert':
          if (data?.type === 'notify') {
            ...
            if (mensagem && !mensagem.key?.fromMe) {
              const from = mensagem.key?.remoteJid?.replace('@s.whatsapp.net', '');
              const text = mensagem.message?.conversation || 
  ...
  // 13. STATUS PÚBLICO
  async getPublicStatus(): Promise<{connected: boolean, qrCode?: string, profileName?: string}> {
    const connected = await this.checkConnection();
