...
export async function registerRoutes(app: express.Application) {
  ...
  app.post("/api/ai-pro/analyze-file", upload.single('file'), async (req, res) => {
    try {
      if (!req.file) {
  ...
  app.post("/api/ai-pro/analyze-competitors", async (req, res) => {
    try {
      ...
      console.log(`📊 Iniciando análise de concorrentes para: ${industry}`);

      const analysis = await AIAnalysisService.analyzeCompetitors(industry, location);

      res.json({
        success: true,
        analysis: analysis,
        timestamp: new Date().toISOString(),
        message: `Análise de concorrentes concluída para ${industry}`
      });
  ...
  app.post("/api/ai-pro/tech-recommendations", async (req, res) => {
    try {
      ...
      console.log(`🎯 Gerando recomendações tecnológicas para: ${userData.name || 'Cliente'}`);

      const recommendations = AIAnalysisService.generateTechRecommendations(userData);

      res.json({
        success: true,
        recommendations: recommendations,
        count: recommendations.length,
        message: "Recomendações tecnológicas geradas com sucesso"
      });
  ...
  app.get("/api/ai-pro/available-meetings", async (req, res) => {
    try {
      ...
      console.log(`📅 Gerando agenda de reuniões (urgência: ${urgency})`);
      ...
      res.json({
        ...
        message: "Agenda de reuniões gerada com sucesso"
      });
  ...
  app.post("/api/ai-pro/schedule-meeting", async (req, res) => {
    try {
      const { meetingId, userData, meetingType } = req.body;

      if (!meetingId || !userData || !userData.email) {
        return res.status(400).json({
          success: false,
          error: "ID da reunião e dados do usuário são obrigatórios"
        });
      }

      console.log(`📅 Agendando reunião: ${meetingId} para ${userData.name}`);

      // Simular agendamento (integrar com calendário real)
      ...
      // - Outlook Calendar
      // - Calendly
      // - Sistema interno de agendamento
      ...
      res.json({
        ...
        instructions: [
          ...
          "Link de reunião será enviado 1h antes"
        ]
      });
  ...
  app.post("/api/ai-pro/calculate-roi", async (req, res) => {
    try {
      ...
      console.log(`💰 Calculando ROI personalizado para: ${userData.name || 'Cliente'}`);

      // Simular cálculo avançado de ROI
      const employeeCount = parseInt(userData.employees?.split('-')[0] || '5');
      const businessType = userData.businessType?.toLowerCase() || '';
      
      const roiData = {
        currentCosts: employeeCount * 120 * 12, // €120/mês por funcionário
        projectedSavings: employeeCount * 120 * 12 * 0.3, // 30% de economia
        implementationCost: 5000 + (employeeCount * 200), // Custo base + por funcionário
        paybackPeriod: 8, // meses
        roi: 180, // %
        annualSavings: employeeCount * 120 * 12 * 0.3,
        breakdown: {
      ...
      res.json({
        success: true,
        roi: roiData,
        message: "Análise de ROI concluída"
      });
  ...
  // === ROTAS EXISTENTES (Baileys WhatsApp) ===
      ...
      if (whatsappBaileys.getConnectionStatus()) {
        ...
        if (sendPDF && userData) {
          ...
          try {
            ...
            success = await whatsappBaileys.sendMessageWithPDFs(number, message, [
              {
                buffer: portfolioPDF,
                fileName: `Construware-Portfolio-${userData.name || "Cliente"}.pdf`
              },
              {
                buffer: reportPDF,
                fileName: `Relatorio-Personalizado-${userData.name || "Cliente"}.pdf`
              }
            ]);
        ...
        return res.json({
          ...
          message: "WhatsApp não conectado. Verifique o QR Code nos logs do servidor.",
          instruction: "Escaneie o QR Code que aparece nos logs do servidor para conectar o WhatsApp",
        });
      }
    } catch (error) {
    ...
    try {
      ...
      res.json({
        ...
        baileys: {
          ...
          status: isConnected ? "Conectado" : "Aguardando conexão (QR Code nos logs)"
        }
      });
    } catch (error) {
      res.status(500).json({
