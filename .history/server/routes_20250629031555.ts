...
export async function registerRoutes(app: express.Application) {
  ...
  app.post("/api/whatsapp/send-demo", async (req, res) => {
    try {
      ...
      console.log(`📱 [DEMO-${type?.toUpperCase()}] Enviando para: ${phone}`);

      // Verificar se Baileys está conectado
      if (whatsappBaileys.getConnectionStatus()) {
        console.log(
          `✅ [DEMO-${type?.toUpperCase()}] WhatsApp conectado, enviando...`
        );

        // Usar o método direto do Baileys que sabemos que funciona
        const success = await whatsappBaileys.sendMessage(phone, message);

        if (success) {
          console.log(
            `✅ [DEMO-${type?.toUpperCase()}] Mensagem enviada com sucesso!`
          );
          return res.json({
            success: true,
            method: "baileys",
            message: `Demo ${type} enviada via Baileys`,
          });
        } else {
          throw new Error(`Falha no envio da demo ${type} via Baileys`);
        }
      } else {
        console.log(
          `⚠️ [DEMO-${type?.toUpperCase()}] Baileys não conectado...`
        );
        return res.json({
          success: false,
          method: "baileys_not_connected",
      ...
      console.error(`❌ [DEMO-${type?.toUpperCase()}] Erro na rota:`, error);
