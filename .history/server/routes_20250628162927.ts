...
export async function registerRoutes(app: express.Application) {
  ...
  app.post("/api/whatsapp/send-report", async (req, res) => {
    try {
      const { phone, message, clientData } = req.body;
      
      console.log('📱 Enviando relatório IA via WhatsApp:', { phone, clientData });
      
      // Import Baileys functions (assuming they're available)
      ...
      if (result.success) {
        console.log('✅ Relatório IA enviado via WhatsApp com sucesso!');
        res.json({ 
          success: true, 
        ...
        console.error('❌ Erro ao enviar relatório via WhatsApp:', result.error);
        res.status(500).json({ 
          success: false, 
      ...
      console.error('❌ Erro na API WhatsApp Report:', error);
      res.status(500).json({ 
        success: false, 
