...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/whatsapp/send", async (req, res) => {
    try {
      const { number, message, userData } = req.body;
      ...
      const result = await evolutionAPI.sendMessage(number, message, { userData });
      ...
      if (result.success) {
        ...
        res.status(500).json({
          success: false,
          error: result.error || 'Falha no envio via Evolution API'
        });
      }
    } catch (error) {
  ...
  app.get("/api/whatsapp/status", async (req, res) => {
    try {
      const status = evolutionAPI.getStatus();
      res.json(status);
    } catch (error) {
      console.error('❌ Erro ao verificar status:', error);
      res.status(500).json({
        success: false,
        error: 'Erro ao verificar status da Evolution API'
      });
    }
  });
  ...
  app.post("/api/generate-brand-pdf", async (req, res) => {
    try {
      ...
      const pdfBuffer = await generateHighQualityPDF(generateCompanyPortfolioHTML());
  ...
  app.post("/api/generate-report-pdf", async (req, res) => {
    try {
      ...
      const pdfBuffer = await generateHighQualityPDF(generatePersonalizedReportHTML(clientData));
...
function generateRecommendation(userData: any): string {
  const recommendations = {
    'Construção': 'Sistema ERP especializado para construção com gestão de obras, materiais, equipamentos e relatórios de progresso em tempo real. Inclui módulos de orçamentação automática, controlo de custos e dashboards executivos para tomada de decisão estratégica.',
    'Consultoria': 'Plataforma CRM avançada com gestão de projetos, time tracking automático, faturação inteligente e dashboards de performance. Sistema otimizado para equipas de consultoria com automação de follow-ups e relatórios de produtividade.',
    'E-commerce': 'Solução e-commerce completa com gestão de stock automática, analytics avançados, marketing automation e integração multi-canal. Inclui sistema de recomendações IA e otimização de conversões.',
    'Restaurante': 'Sistema POS integrado com gestão de delivery, controlo de ingredientes, reservas automáticas e análises de vendas específicas para restauração. Inclui integração com aplicações de delivery e gestão de mesas.',
    'Serviços': 'Plataforma de gestão de serviços com agendamento inteligente, gestão de técnicos, histórico completo de clientes e faturação automática. Sistema otimizado para empresas de serviços com workflow personalizado.',
    'Outro': 'Sistema personalizado desenvolvido especificamente para as necessidades do seu setor, com funcionalidades sob medida e integração com sistemas existentes. Análise detalhada das suas necessidades específicas.'
  };
