...
import { baileys } from "./baileys-service";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/whatsapp/send", async (req, res) => {
    try {
      ...
      const result = await evolutionAPI.sendMessage(number, message, {
        userData,
      });
  ...
  // Nova rota: Baileys como método principal (anti-ban)
  app.post("/api/whatsapp/send-baileys", async (req, res) => {
    try {
      const { number, message, userData, sendPDF = false } = req.body;
      ...
      if (baileys.isConnected()) {
        ...
        if (sendPDF && userData) {
          ...
          try {
            ...
            // Aguardar 3s antes de enviar PDFs (anti-spam)
            await new Promise(resolve => setTimeout(resolve, 3000));
            
            // Enviar Portfolio
            await baileys.sendPDF(
              number,
              "📄 Portfolio Construware - Casos de Sucesso e Serviços",
              portfolioPDF,
              `Construware-Portfolio-${userData.name || 'Cliente'}.pdf`
            );
            ...
            await baileys.sendPDF(
              number,
              "📊 Análise Personalizada - Recomendações Específicas",
              reportPDF,
              `Relatorio-Personalizado-${userData.name || 'Cliente'}.pdf`
            );
            ...
            success = await baileys.sendMessage(number, message);
          }
        } else {
        ...
        console.log("⚠️ [BAILEYS] Não conectado, usando fallback...");
        ...
        if (result.success) {
          return res.json({
            ...
            messageId: result.messageId,
            message: "Enviado via Evolution API (fallback)",
          });
        } else {
          throw new Error(result.error || "Falha no fallback");
        }
      }
      ...
      const waLink = `https://wa.me/${number.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(message)}`;
