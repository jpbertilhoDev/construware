...
import fs from "fs";
import P from "pino"; // Logger recomendado pela documentação oficial
...
class WhatsAppBaileys {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log(
        "🔍 Baileys object keys:",
        Object.keys(this.baileys).slice(0, 20)
      );
      ...
      if (typeof makeWASocket !== "function") {
        throw new Error(
          `makeWASocket não encontrado. Propriedades disponíveis: ${Object.keys(
            this.baileys
          ).join(", ")}`
        );
      }
      ...
      if (this.baileys.useMultiFileAuthState) {
        const authResult = await this.baileys.useMultiFileAuthState(
          "./baileys_auth"
        );
      ...
      // Criar logger conforme documentação oficial
      const logger = P({ level: "silent" });

      // Criar socket conforme documentação oficial
      const config: any = {
        logger: logger, // Usar logger pino
      ...
      if (this.baileys.Browsers?.ubuntu) {
        config.browser = this.baileys.Browsers.ubuntu(
          "Construware Business Bot"
        );
      } else {
      ...
      console.log(
        "✅ Baileys configurado com sucesso seguindo documentação oficial!"
      );
    } catch (error) {
  ...
  private async handleConnectionUpdate(update: any) {
    ...
    if (qr) {
      ...
      console.log("🔍 Use este comando para exibir o QR:");
      console.log(
        `node -e "const qr=require('qrcode-terminal'); qr.generate('${qr}', {small: true});"`
      );
    }
    ...
    try {
      ...
      if (this.sock.onWhatsApp) {
        try {
          ...
          console.log(
            "⚠️ Não foi possível verificar o número, enviando mesmo assim..."
          );
        }
      }
...
export async function sendWhatsAppMessage(
  ...
  try {
    ...
    const result = await whatsappBaileys.sendMessage(phoneNumber, message);
