...
export class ConstruwarePDFGenerator {
  ...
  static async generatePersonalizedReport(clientData: ClientData): Promise<Buffer> {
    const doc = new jsPDF('p', 'mm', 'a4');
  ...
  private static addPersonalizedHeader(doc: jsPDF, clientData: ClientData) {
    ...
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text(`RELATÓRIO PERSONALIZADO - ${clientData.name.toUpperCase()}`, 20, 40);
  }
...
personalizadas que geram resultados mensuráveis e sustentáveis.`;
    ...
    const services = [
      {
        ...
        price: 'GRATUITO',
        features: ['Análise de processos', 'Recomendações personalizadas', 'ROI estimado']
      },
      {
        name: 'Sistema Básico',
        description: 'Solução para empresas até 20 funcionários',
        price: 'A partir de €2.500',
        features: ['Sistema personalizado', 'Formação incluída', 'Suporte 6 meses']
      },
      {
  ...
  private static addContactsAndNextSteps(doc: jsPDF) {
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text('Entre em contacto connosco e receba uma proposta personalizada', 45, y + 20);
  }
  ...
  private static addRecommendedSolution(doc: jsPDF, clientData: ClientData) {
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    const solutionLines = doc.splitTextToSize(clientData.recommendedSolution, 160);
