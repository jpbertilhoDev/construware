...
Equipa Construware
  ...
  const tryAutoSendWhatsApp = async (phone: string, message: string): Promise<boolean> => {
    // Método 1: CallMeBot API (gratuita)
    try {
      const callMeBotUrl = `https://api.callmebot.com/whatsapp.php?phone=${phone}&text=${encodeURIComponent(message)}&apikey=1234567`;
      ...
      return true; // Se chegou até aqui, provavelmente funcionou
    } catch (error) {
      console.log('CallMeBot falhou, tentando método 2...');
    }

    // Método 2: WhatsApp Web API (alternativo)
    try {
      const whatsWebUrl = `https://web.whatsapp.com/send?phone=${phone}&text=${encodeURIComponent(message)}&app_absent=0`;
      
      // Abrir numa nova janela pequena que se fecha automaticamente
      const popup = window.open(whatsWebUrl, 'whatsapp', 'width=400,height=600');
      
      // Simular clique no botão enviar (não é 100% confiável)
      setTimeout(() => {
        if (popup) {
          popup.close();
        }
      ...
      console.log('WhatsApp Web falhou, tentando método 3...');
    }

    // Método 3: Usando serviço próprio via webhook
    try {
      // Este seria um webhook seu ou um serviço terceirizado
      const webhookUrl = 'https://your-webhook.com/send-whatsapp';
      ...
      console.log('Webhook falhou, usando fallback...');
    }
