...
*Data: ${new Date().toLocaleDateString('pt-PT')} às ${new Date().toLocaleTimeString('pt-PT')}*`;
  ...
  const sendWhatsAppToClientAndTeam = async (userData: UserData, recommendations: string) => {
    try {
      ...
      // Generate fixed Google Meet link for CLIENT (same room for all demos)
      const googleMeetLink = `https://meet.google.com/construware-demo-${meetDate.getDate()}${meetDate.getMonth()}`;
