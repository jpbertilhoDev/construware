...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  const getCurrentColor = () => {
    ...
    console.log('Timeline Debug:', { visibleItems, dataLength: data.length, progress });
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const progress = Math.max(0, Math.min(1, (scrollTop - containerTop + window.innerHeight * 0.5) / containerHeight));
      const newVisibleItems = Math.ceil(progress * data.length);
      
      console.log('Scroll Debug:', { scrollTop, containerTop, progress, newVisibleItems });
  ...
  useEffect(() => {
    // Simple timer to test color changes
    const timer = setInterval(() => {
      setVisibleItems(prev => {
        const next = prev >= data.length ? 1 : prev + 1;
        console.log('Timer update:', next);
        return next;
      });
    ...
    >
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-12 sm:pb-16 md:pb-20">
        ...
        <div className="absolute left-6 sm:left-8 md:left-8 top-0 overflow-hidden w-[2px]">
          <motion.div
            className={`absolute inset-x-0 top-0 w-[2px] rounded-full transition-all duration-500 ${getCurrentColor().gradient} ${getCurrentColor().shadow}`}
            initial={{ height: "20%" }}
            animate={{ height: `${Math.max(20, (visibleItems / data.length) * 100)}%` }}
