...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  const getColorOpacity = (colorIndex: number) => {
    const progress = Math.max(0.01, visibleItems / data.length); // Ensure minimum progress
    const sectionProgress = progress * 3; // Convert to 0-3 range
    
    if (colorIndex === 0) {
      // Orange: Full opacity at start, fade out as we approach purple
      if (sectionProgress <= 1.0) return 1;
      if (sectionProgress <= 1.4) return 1 - ((sectionProgress - 1.0) / 0.4);
      return 0;
    } else if (colorIndex === 1) {
      // Purple: Fade in before section 1, fade out before section 2
      if (sectionProgress <= 1.0) return 0;
      if (sectionProgress <= 1.4) return (sectionProgress - 1.0) / 0.4;
      if (sectionProgress <= 2.0) return 1;
      if (sectionProgress <= 2.4) return 1 - ((sectionProgress - 2.0) / 0.4);
      return 0;
    } else {
      // Green: Fade in before section 2
      if (sectionProgress <= 2.0) return 0;
      if (sectionProgress <= 2.4) return (sectionProgress - 2.0) / 0.4;
      return 1;
    }
  };
