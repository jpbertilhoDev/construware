  ...
  return (
    ...
    >
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-12 sm:pb-16 md:pb-20">
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-20 sm:top-32 md:top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-8 sm:h-10 absolute left-2 sm:left-3 md:left-3 w-8 sm:w-10 rounded-full bg-black border-2 border-orange-500/30 flex items-center justify-center backdrop-blur-sm">
                <div className="h-3 sm:h-4 w-3 sm:w-4 rounded-full bg-gradient-to-r from-orange-400 to-orange-500 border border-orange-300" />
              </div>
        ...
        {/* Optimized animated line */}
        <div className="absolute left-6 sm:left-8 md:left-8 top-0 overflow-hidden w-[2px] bg-[linear-gradient(to_bottom,var(--tw-gradient-stops))] from-transparent from-[0%] via-white/20 to-transparent to-[99%] [mask-image:linear-gradient(to_bottom,transparent_0%,black_10%,black_90%,transparent_100%)]">
          <motion.div
            className="absolute inset-x-0 top-0 w-[2px] bg-gradient-to-t from-orange-500 via-orange-400 to-yellow-400 from-[0%] via-[50%] rounded-full"
            initial={{ height: "0%" }}
            animate={{ height: `${(visibleItems / data.length) * 100}%` }}
