...
export function AIConsultant() {
    ...
    switch (step) {
      ...
      case 7:
        if (deliveryMethod === 'email' || deliveryMethod === 'both') {
          ...
          addMessage(
            `Qual é o seu número de WhatsApp? (formato: +351 9XXXXXXXX)`,
            'ai'
          );
        }
        break;
  ...
  const sendEmailReport = async () => {
    ...
    try {
      ...
      // Inicializar EmailJS (você precisa configurar no site do EmailJS)
      ...
      simulateTyping(() => {
        addMessage(
          `⚠️ **Problema temporário com email automático**\n\n📧 Por favor, contacte-nos diretamente:\n• Email: contato@construware.pt\n• WhatsApp: +351 963 901 577\n\nMencione este código: #IA${Date.now().toString().slice(-6)}`,
          'ai'
        );
      }, 1000);
    }
...
Obrigado por confiar na nossa IA! 🎉`;
    ...
    simulateTyping(() => {
      addMessage(
        `✅ **WhatsApp aberto!**\n\n📱 Relatório pré-preenchido no seu WhatsApp\n👆 Basta clicar em "Enviar" para receber\n\n✨ **Processo completo!**`,
        'ai'
      );
