...
export function AIConsultantAdvanced() {
  ...
  const generatePersonalizedRecommendations = (businessType: any, employeeRange: any, challenge: any): string => {
    ...
    const recommendations: Record<string, string> = {
      'construction': `🏗️ **SOLUÇÃO PARA CONSTRUÇÃO CIVIL**
...
**🎯 RESULTADO: Mais lucro, menos stress!**`,
      ...
      'consulting': `💼 **SOLUÇÃO PARA CONSULTORIA**

**💡 PROBLEMAS QUE RESOLVEMOS:**
• ❌ Clientes perdidos → ✅ Vendas automáticas
• ❌ Tempo perdido → ✅ Mais produtividade
• ❌ Facturas atrasadas → ✅ Pagamentos rápidos

**💰 ECONOMIA: €${savings.total.toLocaleString()}/ano**

**🎯 RESULTADO: Mais clientes, mais lucro!**`,

      'ecommerce': `🛒 **SOLUÇÃO PARA E-COMMERCE**

**💡 PROBLEMAS QUE RESOLVEMOS:**
• ❌ Vendas baixas → ✅ Vendas automáticas 24/7
• ❌ Stock confuso → ✅ Controlo total
• ❌ Clientes perdidos → ✅ Fidelização garantida

**💰 ECONOMIA: €${savings.total.toLocaleString()}/ano**

**🎯 RESULTADO: Vendas que nunca param!**`,

      'restaurant': `🍽️ **SOLUÇÃO PARA RESTAURANTE**

**💡 PROBLEMAS QUE RESOLVEMOS:**
• ❌ Pedidos confusos → ✅ Sistema organizado
• ❌ Desperdício → ✅ Controlo total
• ❌ Clientes insatisfeitos → ✅ Serviço perfeito

**💰 ECONOMIA: €${savings.total.toLocaleString()}/ano**

**🎯 RESULTADO: Restaurante sempre cheio!**`,

      'services': `⚙️ **SOLUÇÃO PARA SERVIÇOS**

**💡 PROBLEMAS QUE RESOLVEMOS:**
• ❌ Agendamentos perdidos → ✅ Agenda organizada
• ❌ Clientes insatisfeitos → ✅ Serviço top
• ❌ Facturas atrasadas → ✅ Pagamentos rápidos

**💰 ECONOMIA: €${savings.total.toLocaleString()}/ano**

**🎯 RESULTADO: Mais clientes, menos trabalho!**`,

      'healthcare': `🏥 **SOLUÇÃO PARA SAÚDE**

**💡 PROBLEMAS QUE RESOLVEMOS:**
• ❌ Consultas atrasadas → ✅ Agenda perfeita
• ❌ Papelada confusa → ✅ Tudo digital
• ❌ Pacientes perdidos → ✅ Atendimento top

**💰 ECONOMIA: €${savings.total.toLocaleString()}/ano**

**🎯 RESULTADO: Pacientes mais satisfeitos!**`,

      'education': `🎓 **SOLUÇÃO PARA EDUCAÇÃO**

**💡 PROBLEMAS QUE RESOLVEMOS:**
• ❌ Aulas confusas → ✅ Ensino digital
• ❌ Alunos desmotivados → ✅ Aprendizagem divertida
• ❌ Pais preocupados → ✅ Acompanhamento total

**💰 ECONOMIA: €${savings.total.toLocaleString()}/ano**

**🎯 RESULTADO: Alunos mais inteligentes!**`,

      'other': `🏢 **SOLUÇÃO PERSONALIZADA**

**💡 PROBLEMAS QUE RESOLVEMOS:**
• ❌ Processos lentos → ✅ Automação rápida
• ❌ Custos altos → ✅ Economia garantida
• ❌ Stress diário → ✅ Tranquilidade total

**💰 ECONOMIA: €${savings.total.toLocaleString()}/ano**

**🎯 RESULTADO: Negócio que funciona sozinho!**`
    };
    ...
    switch (step) {
      ...
      case 7:
        ...
        simulateTyping(() => {
          ...
          // Send WhatsApp to both client and team
          sendWhatsAppToClientAndTeam(updatedUserData, recommendations).then((success) => {
            setTimeout(() => {
              if (success) {
                addMessage(
...
*Data: ${new Date().toLocaleDateString('pt-PT')} às ${new Date().toLocaleTimeString('pt-PT')}*`;
  ...
  const sendWhatsAppToClientAndTeam = async (userData: UserData, recommendations: string) => {
    try {
      // Generate Google Meet link for tomorrow 2 PM
      const meetDate = new Date();
      meetDate.setDate(meetDate.getDate() + 1);
      meetDate.setHours(14, 0, 0, 0);
      
      const meetLink = generateCalendarLink('Google Meet', 30);
...
Equipa Construware`;
      ...
      // Message for TEAM
      const teamMessage = `🚨 *NOVO LEAD QUALIFICADO - IA*
