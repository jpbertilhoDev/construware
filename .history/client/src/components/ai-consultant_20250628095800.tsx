...
🚨 *LEAD QUALIFICADO - SEGUIMENTO PRIORITÁRIO!*`;
  ...
  const sendWhatsAppToClient = async () => {
    ...
    try {
      ...
      if (result.success) {
        console.log(`✅ Sucesso via ${result.metodo}:`, result);
        
        if (result.metodo === 'evolution') {
          // Enviado automaticamente via Evolution API
          addMessage(
            `✅ **Mensagem enviada automaticamente!**\n\n📱 **Número confirmado:** ${result.numeroFormatado}\n🤖 **Método:** Evolution API (Automático)\n📨 **Message ID:** ${result.messageId}\n💰 **Custo:** ${result.custo}\n\n🎉 **O relatório personalizado foi enviado diretamente para o seu WhatsApp!**\n\n📱 **Verifique seu WhatsApp agora** - a mensagem já chegou com todos os detalhes da análise.\n\n⚡ **BONUS:** Nossa equipa comercial foi notificada e entrará em contacto nas próximas 24h!`,
            'ai'
          );
        } else if (result.metodo === 'fallback' && result.whatsappUrl) {
          // Usar fallback wa.me
          window.open(result.whatsappUrl, '_blank');
          
          addMessage(
            `✅ **WhatsApp aberto com sucesso!**\n\n📱 **Número confirmado:** ${result.numeroFormatado}\n🔄 **Método:** wa.me (Fallback)\n📄 **Relatório personalizado** pré-preenchido\n\n🎯 **COMO RECEBER O RELATÓRIO:**\n\n1️⃣ O WhatsApp abriu numa nova janela/aba\n2️⃣ Clique em **"Enviar"** para receber o relatório completo\n3️⃣ Guarde a mensagem para referência futura\n\n💼 **O QUE RECEBERÁ:**\n• Análise completa do perfil da sua empresa\n• Recomendações personalizadas pela IA\n• Economia estimada com automação\n• Próximos passos detalhados\n• Contactos diretos da nossa equipa\n\n⚡ **BONUS:** Nossa equipa comercial também foi notificada!`,
            'ai'
          );
        }
        
        // Notificar equipa sempre
        await notifyTeamWhatsAppRequest(result.numeroFormatado || userData.whatsapp, userData.name || 'Cliente');
  ...
  const sendWhatsAppFallback = async () => {
    ...
    // Adicionar dicas específicas do setor
    const getSectorSpecificTips = (businessType: any): string => {
      const tips = {
        'Construção': '🏗️ *Dicas específicas para Construção:*\n• Gestão de obras em tempo real\n• Controlo de materiais e equipamentos\n• Orçamentos automáticos\n• Relatórios de progresso',
        'Consultoria': '💼 *Dicas específicas para Consultoria:*\n• CRM avançado para clientes\n• Gestão de projetos complexos\n• Time tracking automático\n• Faturação inteligente',
        'E-commerce': '🛒 *Dicas específicas para E-commerce:*\n• Gestão de stock automática\n• Analytics de vendas avançados\n• Marketing automation\n• Integração multi-canal',
        'Restaurante': '🍽️ *Dicas específicas para Restauração:*\n• Sistema POS integrado\n• Gestão de delivery\n• Controlo de ingredientes\n• Reservas automáticas',
        'Serviços': '⚙️ *Dicas específicas para Serviços:*\n• Agendamento inteligente\n• Gestão de técnicos\n• Histórico de clientes\n• Faturação automática',
        'Outro': '🎯 *Soluções personalizadas:*\n• Análise específica do seu setor\n• Funcionalidades sob medida\n• Integração com sistemas existentes\n• Suporte especializado'
      };
      
      return tips[businessType?.name || 'Outro'] || tips['Outro'];
    };

    const sectorTips = getSectorSpecificTips(businessTypeInfo);

    // Mensagem personalizada profissional
    const report = `🤖 *CONSTRUWARE - RELATÓRIO IA PERSONALIZADO*
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
🏢 Setor: *${businessTypeInfo?.name || 'Não especificado'}*
👥 Funcionários: *${userData.employees || 'Não especificado'}*
⚠️ Principal Desafio: *${userData.mainChallenge || 'Não especificado'}*
💻 Sistema Atual: *${userData.currentSystem || 'Não especificado'}*

💰 *POTENCIAL DE ECONOMIA ANUAL:*
*€${savings.toLocaleString()}* com automação e otimização

🎯 *SOLUÇÃO RECOMENDADA:*
${recommendations}

Esta recomendação foi gerada pela nossa IA com base no perfil específico da sua empresa.
...
1️⃣ *Demonstração Gratuita (30min)*
   ...
   → Mostrar funcionalidades específicas para ${businessTypeInfo?.name?.toLowerCase() || 'seu setor'}
...
3️⃣ *Proposta Comercial Personalizada*
   → Orçamento adequado ao seu orçamento
...
${userData.email ? `📧 *Relatório técnico completo* enviado para: ${userData.email}` : ''}
...
*Obrigado por confiar na nossa IA!* 🎉✨`;
    ...
    simulateTyping(() => {
      addMessage(
        `✅ **WhatsApp aberto com sucesso!**\n\n📱 **Número confirmado:** +${formattedPhone}\n📄 **Relatório personalizado** pré-preenchido\n\n🎯 **COMO RECEBER O RELATÓRIO:**\n\n1️⃣ O WhatsApp abriu numa nova janela/aba\n2️⃣ Clique em **"Enviar"** para receber o relatório completo\n3️⃣ Guarde a mensagem para referência futura\n\n💼 **O QUE RECEBERÁ:**\n• Análise completa do perfil da sua empresa\n• Recomendações personalizadas pela IA\n• Economia estimada com automação\n• Próximos passos detalhados\n• Contactos diretos da nossa equipa\n\n⚡ **BONUS:** Nossa equipa comercial também foi notificada e entrará em contacto nas próximas 24h com uma demonstração personalizada gratuita!`,
        'ai'
      );
