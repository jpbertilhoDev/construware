...
*Data: ${new Date().toLocaleDateString('pt-PT')} às ${new Date().toLocaleTimeString('pt-PT')}*`;
  ...
  const sendWhatsAppToClientAndTeam = async (userData: UserData, recommendations: string) => {
    try {
      ...
      // Generate different links for client and team
      const clientMeetLink = generateCalendarLink('Google Meet', 30); // Simple join link
      const teamEditLink = generateCalendarEditLink(userData, meetDate); // Full edit link
      
      // Message for CLIENT - Simple join link
...
💡 *Basta clicar no link para adicionar à sua agenda!*

Até amanhã! 🚀
*Equipa Construware*`;

      // Message for TEAM - Edit/manage link
...
📅 *CLIQUE PARA CRIAR/EDITAR REUNIÃO:*
${teamEditLink}
...
✏️ *Use o link acima para criar e editar a reunião*`;
