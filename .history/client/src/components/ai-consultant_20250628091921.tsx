...
Equipa Construware
  ...
  const sendWhatsAppToClient = () => {
    console.log('📱 sendWhatsAppToClient called', { whatsapp: userData.whatsapp, deliveryMethod });
    
    if (!userData.whatsapp) {
      console.error('❌ WhatsApp não definido!', userData);
      addMessage(
...
Obrigado por confiar na nossa IA! 🎉`;
    ...
    const phoneNumber = userData.whatsapp.replace(/\D/g, '');
    ...
    simulateTyping(() => {
      addMessage(
        `✅ **WhatsApp aberto com sucesso!**\n\n📱 Relatório personalizado pré-preenchido\n👆 **Basta clicar em "Enviar"** para receber o seu relatório\n\n🎯 O WhatsApp deve ter aberto numa nova janela`,
        'ai'
      );
...
🎯 Cliente pré-qualificado pela IA - Contactar em 24h!`;
  ...
  const handleTextInput = (e: React.FormEvent) => {
    ...
    switch (currentStep) {
      ...
      case 7:
        if (deliveryMethod === 'email' || deliveryMethod === 'both') {
          console.log('📧 Capturing email:', response);
          handleUserResponse(response, { email: response });
        } else {
          console.log('📱 Capturing WhatsApp:', response, 'delivery method:', deliveryMethod);
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-white/80 text-sm">
                        Construware Assistant {deliveryMethod && `• ${deliveryMethod}`}
                      </p>
                    </div>
                  </div>
                  <Button
                              ...
                              {message.options.map((option, index) => (
                                <Button
                                  ...
                                  if (currentStep === 2) {
                                    ...
                                    console.log('🎯 Delivery method set:', method);
