...
*Data: ${new Date().toLocaleDateString('pt-PT')} às ${new Date().toLocaleTimeString('pt-PT')}*`;
  ...
  const sendWhatsAppToClientAndTeam = async (userData: UserData, recommendations: string) => {
    try {
      ...
      // Generate unique Google Meet room for this specific demo
      const meetingCode = `construware-demo-${userData.name?.toLowerCase().replace(/\s+/g, '')}-${meetDate.getDate()}${meetDate.getMonth()}`;
      ...
      // Message for CLIENT - Direct Google Meet link
